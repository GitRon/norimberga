...
@pytest.mark.django_db
def test_tile_demolish_view_allows_ruins_demolition(user, ruins_building):
    """Test TileDemolishView allows demolishing ruins despite being unique."""
    savegame = SavegameFactory(user=user, is_active=True, coins=50)
    tile = TileFactory(building=ruins_building, savegame=savegame)
    ...
    # Verify ruins were removed
    tile.refresh_from_db()
    assert tile.building is None

    # Verify demolition costs were charged (ruins cost 20)
    savegame.refresh_from_db()
    assert savegame.coins == 30  # 50 - 20 = 30
...
@pytest.mark.django_db
def test_tile_demolish_view_insufficient_coins_for_ruins(user, ruins_building):
    """Test TileDemolishView rejects ruins demolition when insufficient coins."""
    savegame = SavegameFactory(user=user, is_active=True, coins=10)
    tile = TileFactory(building=ruins_building, savegame=savegame)
