...
from apps.city.events.events.civic_pride import Event as CivicPrideEvent
from apps.savegame.tests.factories import SavegameFactory


@pytest.mark.django_db
def test_civic_pride_event_init():
    """Test CivicPrideEvent initialization and class attributes."""
    savegame = SavegameFactory(population=100)

    event = CivicPrideEvent(savegame=savegame)

    assert event.PROBABILITY == 15
    assert event.LEVEL == messages.INFO
    assert event.TITLE == "Civic Pride"
    assert event.savegame.id == savegame.id


@pytest.mark.django_db
def test_civic_pride_event_get_probability_with_population():
...
@pytest.mark.django_db
def test_civic_pride_event_get_probability_zero_population():
...
@pytest.mark.django_db
def test_civic_pride_event_get_verbose_text():
    ...
    expected_text = 'Someone paints "Best Town in the Realm" on the gate. A neighboring town paints "Second Best."'
    assert verbose_text == expected_text


@pytest.mark.django_db
def test_civic_pride_event_get_effects():
...
@pytest.mark.django_db
def test_civic_pride_event_process():
    ...
    assert "Someone paints" in result_text
    assert "Best Town in the Realm" in result_text
